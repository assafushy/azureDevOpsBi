...
async function AddViewFilter(req, res) {
  console.log(
    `Adding view filter: ${JSON.stringify(
      req.swagger.params.newViewFilter.value
    )}`
  );
  let newFilter = {};
  try {
    newFilter = await viewFilters.create(
      req.swagger.params.newViewFilter.value
    );
  } catch (error) {
    console.log(error);
...
async function DeleteViewFilterById(req, res) {
  ...
  let deletedFilter;
  try {
    deletedFilter = await viewFilters.findByIdAndDelete(
...
async function UpdateViewFilterById(req, res) {
  ...
  try {
    updatedViewFilter = await viewFilters.findByIdAndUpdate(
      req.swagger.params.id.value,
      req.swagger.params.updatedViewFilter.value
    );
  } catch (error) {
    console.log(error);
    res.statusCode = 400;
